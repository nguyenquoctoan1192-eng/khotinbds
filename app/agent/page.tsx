...
export default function AgentPage() {
  ...
  const buildListingUrl = (listingId: string) => {
    ...
    return query
  ? `/listing/${listingId}?view=agent&${query}`
  : `/listing/${listingId}?view=agent`;
  };
    ...
    : [
        ...
        .join(" - ");
                  ...
                  <div style={{ display: "flex", flexDirection: isMobile ? "row" : "column", gap: 10, justifyContent: isMobile ? "flex-start" : "flex-end", width: isMobile ? "100%" : "auto", marginTop: isMobile ? 10 : 0, flexShrink: 0, flexWrap: "wrap" }}>
                    <button style={{ background: "#111827", color: "#fff", border: "none", padding: "12px 18px", borderRadius: 10, cursor: "pointer", fontWeight: "bold" }} onClick={() => router.push(`/listing/${listing.id}?view=agent`)}>
                      Xem chi tiết
                    </button>
                  </div>
                </div>
