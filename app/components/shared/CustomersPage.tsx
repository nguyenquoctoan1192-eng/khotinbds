...
function CustomerCard({
  ...
  const searchHref = requirementQuery
    ? `${homeHref}?q=${encodeURIComponent(requirementQuery)}`
    : homeHref;
...
function CustomersContent({ mode }: { mode: AccessMode }) {
  const router = useRouter();
  const homeHref = mode === "admin" ? "/admin" : "/agent";
...
export default function CustomersPage({ mode }: { mode: AccessMode }) {
  return (
    <RoleGate allowedRoles={["admin", "agent"]}>
      <CustomersContent mode={mode} />
    </RoleGate>
  );
}
