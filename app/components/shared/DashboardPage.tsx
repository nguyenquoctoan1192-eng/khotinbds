...
const getLeads = async (profile: ServerProfile, mode: DashboardMode) => {
  ...
  let leadQuery = supabase
    .from("leads")
    .select("id, fullname, phone, preferred_districts, note, max_price, status, lead_score, lead_temperature, created_at, assigned_to")
    .order("created_at", { ascending: false });
  if (mode === "agent") leadQuery = leadQuery.eq("assigned_to", profile.id);
  ...
  if (
    ...
    let fallbackQuery = supabase
      ...
      .order("created_at", { ascending: false });
    if (mode === "agent") fallbackQuery = fallbackQuery.eq("assigned_to", profile.id);
        ...
        <div style={{ display: "grid", gap: 10 }}>
          {items.slice(0, 6).map((item) => (
            <Link
              ...
              href={`${customersHref}#lead-${item.lead.id}`}
              style={{
                display: "grid",
                gap: 4,
                color: "#111827",
...
type DashboardMode = "admin" | "agent";
...
export default async function DashboardPage({
  ...
  if (mode === "agent" && profile.role !== "agent") {
    redirect("/admin/dashboard");
  }

  const { leads, activities, error } = await getLeads(profile, mode);
    ...
    <div style={{ fontFamily: "var(--font-inter)", minHeight: "100vh", background: "#f3f4f6" }}>
      ...
      <main style={{ maxWidth: 1180, margin: "0 auto", padding: 20 }}>
        ...
        <section style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(260px, 1fr))", gap: 16, marginBottom: 22 }}>
          <NextActionList title="Leads cần chăm sóc hôm nay" items={aiFollowUpDueItems} customersHref={customersHref} />
          <NextActionList title="Việc cần làm hôm nay" items={todaysActions} customersHref={customersHref} />
          <NextActionList title="Cần gọi ngay" items={callNowItems} customersHref={customersHref} />
          <NextActionList title="Cần follow-up" items={followUpItems} customersHref={customersHref} />
          <NextActionList title="Chờ phản hồi" items={waitingItems} customersHref={customersHref} />
        </section>
